"""
Useful tool to run the test suite for olddjangorestframework and generate a coverage report.
"""
...
import os
import sys
os.environ['DJANGO_SETTINGS_MODULE'] = 'olddjangorestframework.runtests.settings'

from coverage import coverage
from itertools import chain

def main():
    """Run the tests for olddjangorestframework and generate a coverage report."""
    ...
    if hasattr(TestRunner, 'func_name'):
        ...
        failures = TestRunner(['olddjangorestframework'])
    else:
        test_runner = TestRunner()
        failures = test_runner.run_tests(['olddjangorestframework'])
    cov.stop()

    # Discover the list of all modules that we should test coverage for
    import olddjangorestframework

    project_dir = os.path.dirname(olddjangorestframework.__file__)
    cov_files = []
