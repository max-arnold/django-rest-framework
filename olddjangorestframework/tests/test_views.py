...
from olddjangorestframework.parsers import JSONParser
...
class BaseViewTests(TestCase):
    """Test the base view class of olddjangorestframework"""
    urls = 'olddjangorestframework.tests.test_views'
...
class ExtraViewsTests(TestCase):
    """Test the extra views olddjangorestframework provides"""
    urls = 'olddjangorestframework.tests.test_views'

    def test_login_view(self):
        """Ensure the login view exists"""
        response = self.client.get(reverse('olddjangorestframework:login'))
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response['Content-Type'].split(';')[0], 'text/html')

    def test_logout_view(self):
        """Ensure the logout view exists"""
        response = self.client.get(reverse('olddjangorestframework:logout'))
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response['Content-Type'].split(';')[0], 'text/html')
